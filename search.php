...
<main class="chymera-container">
  <div class="chymera-inner">
    ...
    <aside class="search-form-column">
      <form id="chymeraSearchForm" action="/search-process.php" method="post" enctype="multipart/form-data">
...
(function () {
  ...
  function renderAll() {
    ...
    if (mode === 'cutting') {
      html += '<dt>Application</dt><dd>' + (escapeHtml(APPLICATION_LABELS[criteria.application] ?? criteria.application ?? '') || '—') + '</dd>';
      if (app === 'knockout') {
    ...
    if (mode === 'crispri' || mode === 'crispra') {
      if (criteria.inputs?.length) {
        html += '<dt>Search Inputs</dt><dd>' + escapeHtml(criteria.inputs.join(', ')) + '</dd>';
      }
  ...
  // ── Helper: get the currently selected genome value ────────────────────────
  function selectedGenome() {
    return sharedMeta?.genome ?? 'hg38';
  }

  // ── Helper: render a single table cell value ───────────────────────────────
  function renderCell(colName, cell) {
    if (colName === 'Guide Coordinates' && cell != null && cell !== '') {
      const genome = selectedGenome();
      const url    = 'https://genome.ucsc.edu/cgi-bin/hgTracks?db='
                  + encodeURIComponent(genome)
                  + '&position='
        ...
        items.forEach(row => {
          ...
          displayRow.forEach((cell, ci) => {
            ...
            if (ci === 0) {
              ...
              html += '<td' + (hidden ? ' class="col-hidden"' : '') + ' data-column="' + escapeHtml(colName) + '">' + renderCell(colName, cell) + '</td>';
            }
          });
          html += '</tr>';
        });
  ...
  function rebuildColumnMenu() {
    ...
    // Use currentGroup first, then fall back to any loaded group
    const groupsToCheck = currentGroup
      ? [currentGroup, ...enabledGroups.filter(g => g !== currentGroup)]
      : enabledGroups;
  ...
  (function buildSpinner() {
    ...
    el.id = 'searchSpinner';
    el.innerHTML = `<div id="searchSpinnerBox"><img src="/images/beakerspin.svg" alt="" width="72" height="72"><p>Loading…</p></div>`;
  ...
  function openDetailModal(columns, values) {
    ...
    body.innerHTML = columns.map((col, i) =>
      '<dt>' + escapeHtml(col) + '</dt><dd>' + renderCell(col, values[i] ?? '') + '</dd>'
    ).join('');
